            ...
            <Row>
                <Col flex='auto'>
                    <div style={{ float: 'left', margin: '0px 24px 16px 0' }}>
                        {currentUser ? (currentUser.ou === 'modeln' ? <img src={`/modeln-logo-2.png`} alt='Model N Logo' style={{ maxHeight: 25 }} /> : <Typography.Text style={{ color: '#0B588E', textTransform: 'capitalize', fontSize: 32 }} strong>{currentUser?.ou}</Typography.Text>) : null}
                    </div>
                    <Menu mode='horizontal' items={items} selectedKeys={topNavKey} />
                </Col>
                <Col>
                    {
                        isDevMode ? (<Dropdown menu={{ items: peers, onClick: changeBaseUri }}>
